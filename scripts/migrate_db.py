...
def migrate_database():
    ...
    try:
        ...
        if "user_id" not in columns:
            ...
            # Set default user_id for existing records
            db.execute(
                text("ALTER TABLE completions ADD COLUMN user_id TEXT DEFAULT 'default_user'")
            )
            db.execute(
                text("CREATE INDEX IF NOT EXISTS idx_completions_user_id ON completions(user_id)")
            )
            db.commit()
